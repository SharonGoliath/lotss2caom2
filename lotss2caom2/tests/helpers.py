...
def _search_mosaic_id_mock(query_string):
    # the query return value is pyvo.dal.tap.TAPResults, but for the purposes of testing
    # a list of dicts has the same behaviour
    if 'P000' in query_string:
        result = [{
            ...
            'nAxes': 2,
            'pixelSize': 'masked_array(data=[8938, 8938]',
            'pixelScale': 'masked_array(data=[0.0004166669968981296, 0.0004166669968981296]',
            'refFrame': 'ICRS',
            'wcs_equinox': 2000.0,
            'wcs_projection': 'SIN',
            'wcs_refPixel': 'masked_array(data=[4469.0, 4469.0]',
            'wcs_refValues': 'masked_array(data=[0.03125, 23.3953]',
            'wcs_cdmatrix': 'masked_array(data=[-0.0004166669968981296, 0.0, 0.0, 0.0004166669968981296]',
            ...
            'bandpassLo': 1.78448,
            'pixflags': 'Z',
            'coverage': 'masked_array(data=[2.032831406303902, 21.520385451900815, 2.08992790594909, 25.244422592409883, 357.9721112359749, 25.24441664211163, 358.02922050953805, 21.520379666564615]',
            'mosaic_id': 'P000+23',
            'related_products': 'https://vo.astron.nl/lotss_dr2/q/dlmosaic/dlmeta?ID=ivo%3A//astron.nl/%7E%3FLoTSS-DR2/P000%2B23',
            'lofar_obsids': 'masked_array(data=[689778]',
        ...
        result = [{
            ...
            'nAxes':  2,
            'pixelSize':  'masked_array(data=[9264, 9264]',
            'pixelScale': 'masked_array(data=[0.0004166669968981296, 0.0004166669968981296]',
            'refFrame':  'ICRS',
            'wcs_equinox':  2000.0,
            'wcs_projection':  'SIN',
            'wcs_refPixel':  'masked_array(data=[4632.0, 4632.0]',
            'wcs_refValues':  'masked_array(data=[124.678, 61.9895]',
            'wcs_cdmatrix':  'masked_array(data=[-0.0004166669968981296, 0.0, 0.0, 0.0004166669968981296]',
            'bandpassId':  '120-168MHz',
            'bandpassUnit': 'm',
            'bandpassRefval':  2.08189,
            ...
            'bandpassLo':  1.78448,
            'pixflags':  'Z',
            'coverage':  'masked_array(data=[128.5403335917227, 60.00207770393527, 129.06122140208868, 63.85463521484545, 120.2938346473764, 63.85460706549431, 120.81483402706476, 60.002052894353966]',
            'mosaic_id':  'P124+62',
            'related_products':  'https://vo.astron.nl/lotss_dr2/q/dlmosaic/dlmeta?ID=ivo%3A//astron.nl/%7E%3FLoTSS-DR2/P124%2B62',
            'lofar_obsids': 'masked_array(data=[664568]',
